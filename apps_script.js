...
// מפתח אימות לסנכרון (Sync Secret / API Token)
// ניתן להגדיר כאן מחרוזת אקראית או ב-Script Properties תחת השם "SYNC_SECRET".
// אם מוגדר - רק בקשות המצרפות את המפתח יורשו לגשת לנתונים.
...
function handleRequest(e) {
  ...
  try {
    ...
    // אימות סוד הסנכרון (אם הוגדר SYNC_SECRET)
    if (SYNC_SECRET && action !== "ping") {
      ...
      const providedToken = headerToken || (params.sync_secret ? params.sync_secret.toString().trim() : (params.token ? params.token.toString().trim() : ""));
      
      if (providedToken !== SYNC_SECRET) {
        return jsonResponse({ success: false, message: "Unauthorized: Invalid or missing sync secret" }, headers);
      }
    }
